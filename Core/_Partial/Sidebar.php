<?php
    ...
    //Buka setting_acc
    $sql_setting = mysqli_query($conn, "SELECT * FROM setting_acc WHERE akses='$SessionAkses'");
    $data_setting = mysqli_fetch_array($sql_setting);
    $acc_profile=$data_setting['acc_profile'];
    $acc_setting=$data_setting['acc_setting'];
    $acc_akses=$data_setting['acc_akses'];
    $acc_dokter=$data_setting['acc_dokter'];
    $acc_ruang_inap=$data_setting['acc_ruang_inap'];
    $acc_pasien=$data_setting['acc_pasien'];
    $acc_kunjungan=$data_setting['acc_kunjungan'];
    $acc_supplier=$data_setting['acc_supplier'];
    $acc_inventory=$data_setting['acc_inventory'];
    $acc_kasir=$data_setting['acc_kasir'];
    $acc_transaksi=$data_setting['acc_transaksi'];
    $acc_laporan=$data_setting['acc_laporan'];
    $acc_backup=$data_setting['acc_backup'];
?>
<script>
    $(document).ready(function(){
        ...
        $('#SidebarMyProfile').click(function(){
            var Loading='<div class="row text-center"><div class="form-group col col-md-12"><img src="images/loading.gif"></div></div>';
            $('.nav-item').removeClass("bg-inverse-info");
            $('#IdSidebarMyProfile').addClass("bg-inverse-info");
            $('#Halaman').html(Loading);
        ...
        $('#DokterPoliklinik').click(function(){
            var Loading='<div class="row text-center"><div class="form-group col col-md-12"><img src="images/loading.gif"></div></div>';
            $('#Halaman').html(Loading);
            $('.nav-item').removeClass("bg-inverse-info");
            $('#MenuListDokterPoliklinik').addClass("bg-inverse-info");
            $('#Halaman').load('_Page/DokterPoliklinik/DokterPoliklinik.php');
        });
        $('#RuangInap').click(function(){
            var Loading='<div class="row text-center"><div class="form-group col col-md-12"><img src="images/loading.gif"></div></div>';
            $('#Halaman').html(Loading);
            $('.nav-item').removeClass("bg-inverse-info");
            $('#MenuListRuangInap').addClass("bg-inverse-info");
            $('#Halaman').load('_Page/RuangInap/RuangInap.php');
        });
        $('#Pasien').click(function(){
            var Loading='<div class="row text-center"><div class="form-group col col-md-12"><img src="images/loading.gif"></div></div>';
            $('#Halaman').html(Loading);
            $('.nav-item').removeClass("bg-inverse-info");
            $('#MenuListPasien').addClass("bg-inverse-info");
            $('#Halaman').load('_Page/Pasien/Pasien.php');
        });
        $('#Kunjungan').click(function(){
            var Loading='<div class="row text-center"><div class="form-group col col-md-12"><img src="images/loading.gif"></div></div>';
            $('#Halaman').html(Loading);
            $('.nav-item').removeClass("bg-inverse-info");
            $('#MenuListKunjungan').addClass("bg-inverse-info");
            $('#Halaman').load('_Page/Kunjungan/Kunjungan.php');
        });
...
<?php }else{ ?>
    <ul class="nav" id="">
        ...
        <?php if($acc_profile=="Yes"){ ?>
            <li class="nav-item" id="IdSidebarMyProfile">
                <a class="nav-link" href="javascript:void(0);" id="SidebarMyProfile">
                    <i class="menu-icon mdi mdi-account-circle"></i>
                    <span class="menu-title">My Profile</span>
                </a>
            </li>
        <?php }if($acc_setting=="Yes"){ ?>
        ...
        <?php }if($acc_dokter=="Yes"){ ?>
            <li class="nav-item" id="MenuListDokterPoliklinik">
                <a class="nav-link" href="javascript:void(0);" id="DokterPoliklinik">
                    <i class="menu-icon mdi mdi-home-assistant"></i>
                    <span class="menu-title">Dokter & Poliklinik</span>
                </a>
            </li>
        <?php }if($acc_ruang_inap=="Yes"){ ?>
            <li class="nav-item" id="MenuListRuangInap">
                <a class="nav-link" href="javascript:void(0);" id="RuangInap">
                    <i class="menu-icon  mdi mdi-hotel"></i>
                    <span class="menu-title">Ruang Inap</span>
                </a>
            </li>
        <?php }if($acc_pasien=="Yes"){ ?>
            <li class="nav-item" id="MenuListPasien">
                <a class="nav-link" href="javascript:void(0);" id="Pasien">
                    <i class="menu-icon mdi mdi-clipboard-account"></i>
                    <span class="menu-title">Pasien</span>
                </a>
            </li>
        <?php }if($acc_kunjungan=="Yes"){ ?>
            <li class="nav-item" id="MenuListKunjungan">
                <a class="nav-link" href="javascript:void(0);" id="Kunjungan">
                    <i class="menu-icon mdi mdi-human-male-female"></i>
                    <span class="menu-title">Kunjungan/Berobat</span>
                </a>
            </li>
        <?php }if($acc_supplier=="Yes"){ ?>
