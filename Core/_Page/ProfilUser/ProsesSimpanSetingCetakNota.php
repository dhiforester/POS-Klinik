<?php
    ...
    $query_hapus_setting = mysqli_query($conn, "DELETE FROM setting_nota WHERE id_akses='$SessionIdAkses'");
    if($query_hapus_setting){
        //Simpan Setting
        $query_simpan_setting = mysqli_query($conn, "INSERT INTO setting_nota (
            id_akses, 
            tanggal_setting, 
            nama_font, 
            ukuran_font, 
        ...
        if($query_simpan_setting){
            echo "<span class='text-success' id='NotifikasiSimpanSettingNotaBerhasil'>Berhasil</span>";
        }else{
