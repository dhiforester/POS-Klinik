<?php
    ...
    if(empty($_POST['id_dokter'])){
        ...
        $jml_data = mysqli_num_rows(mysqli_query($conn, "SELECT*FROM Kunjungan WHERE id_dokter='$id_dokter'"));
        //Jumlah halaman
...
?>
    <script>
        $(document).ready(function(){
            //ketika klik next
            $('#NextPageHistoryKunjunganDokter').click(function() {
                var valueNext = $('#NextPageHistoryKunjunganDokter').val();
                var mode = valueNext.split(',');
                var page = mode[0];
                var BatasData = mode[1];
                var id_dokter = mode[2];
                $.ajax({
                    url     : "_Page/DokterPoliklinik/HistoryKunjunganDokter.php",
                    method  : "POST",
                    data    : { page: page, BatasData: BatasData, id_dokter: id_dokter },
            ...
            $('#PrevPageHistoryKunjunganDokter').click(function() {
                var ValuePrev = $('#PrevPageHistoryKunjunganDokter').val();
            ...
            ?>
                ...
                $('#PageNumberHistoryKunjunganDokter<?php echo $i;?>').click(function() {
                    var PageNumber = $('#PageNumberHistoryKunjunganDokter<?php echo $i;?>').val();
                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        if(empty($jml_data)){
                            ...
                            $query = mysqli_query($conn, "SELECT*FROM kunjungan WHERE id_dokter='$id_dokter' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
        ...
        <div class="col col-lg-12">
            <form action="javascript:void(0);" id="Paging">
                <div class="btn-group" role="group" aria-label="Basic example">
                    <button type="button" class="btn btn-outline-secondary" id="PrevPageHistoryKunjunganDokter" <?php echo "value='".$prev.",".$batas.",".$id_dokter."'"; ?>>
                        <<
                    </button>
                    <?php 
                        //Navigasi nomor
                        ...
                        for ( $i =$a; $i<=$b; $i++ ){
                    ?>
                    <button type="button" class="<?php if($i==$page){echo "btn btn-primary";}else{echo "btn btn-grey";} ?>" id="PageNumberHistoryKunjunganDokter<?php echo $i;?>" <?php echo "value='".$i.",".$batas.",".$id_dokter."'"; ?>>
                        <?php echo $i;?>
                    </button>
                    <?php 
                        }
                    ?>
                    <button type="button" class="btn btn-outline-secondary" id="NextPageHistoryKunjunganDokter" <?php echo "value='".$next.",".$batas.",".$id_dokter."'"; ?>>
                        >>
                    </button>
                </div>
            </form>
