<?php
    ...
    if(empty($keyword)){
        ...
        $jml_data = mysqli_num_rows(mysqli_query($conn, "SELECT*FROM akses WHERE email like '%$keyword%' OR kontak like '%$keyword%' OR nama like '%$keyword%' OR status like '%$keyword%' OR akses like '%$keyword%'"));
    }
    //Jumlah halaman
        ...
        <table class="table table-bordered table-hover">
            ...
            <tbody>
                <?php
                    ...
                    if(empty($keyword)){
                        $query = mysqli_query($conn, "SELECT*FROM akses ORDER BY id_akses DESC LIMIT $posisi, $batas");
                    }else{
                        $query = mysqli_query($conn, "SELECT*FROM akses WHERE email like '%$keyword%' OR kontak like '%$keyword%' OR nama like '%$keyword%' OR status like '%$keyword%' OR akses like '%$keyword%' ORDER BY id_akses DESC LIMIT $posisi, $batas");
                    }
                    while ($data = mysqli_fetch_array($query)) {
                        ...
                        //Cek setting_acc
                        $CekSettingAcc = mysqli_num_rows(mysqli_query($conn, "SELECT*FROM setting_acc WHERE akses='$akses'"));
                        if($CekSettingAcc>0){
                            $LabelSettingAcc = "badge  badge-inverse-success";
                        }else{
                            $LabelSettingAcc = "badge badge-inverse-danger";
                        }
                ?>
                <tr>
                    <td class="text-center"><?php echo "$no";?></td>
                    <td><?php echo "<small>$nama</small>";?></td>
                    <td><?php echo "<small>$kontak</small>";?></td>
                    <td><?php echo "<small>$email</small>";?></td>
                    <td class="text-center">
                        <small onmousemove="this.style.cursor='pointer'" class="<?php echo ''.$LabelSettingAcc.''; ?>" data-toggle="modal" data-target="#ModalEditAccAskes" data-id="<?php echo ''.$akses.''; ?>">
                            <?php echo ''.$akses.''; ?>
                        </small>
                    </td>
                    ...
                    <td align="center">
                        <div class="btn-group">
                            <button class="btn btn-sm btn-secondary" id="EditUser<?php echo "$no";?>" <?php echo "value='".$id_akses.",".$page.",".$batas."'"; ?> title="Edit Akses">
                                <i class="menu-icon mdi mdi-pencil" aria-hidden="true"></i>
                            </button>
                            <button class="btn btn-sm btn-secondary" data-toggle="modal" data-target="#ModalDeleteUser" data-id="<?php echo $id_akses;?>" title="Hapus Akses">
