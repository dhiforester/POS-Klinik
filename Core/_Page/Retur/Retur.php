<?php
    ...
    if(!empty($_POST['id_transaksi'])){
        ...
        $id_transaksiRetur123=$_POST['id_transaksi'];
...
<script>
    ...
    //Modal ModalRincianRetur
    $('#ModalRincianRetur').on('show.bs.modal', function (e) {
        var Loading='<div class="row text-center"><div class="form-group col col-md-12"><img src="images/loading.gif"></div></div>';
        $('#DetailRincianRetur').html(Loading);
        var id_rincian_retur = $(e.relatedTarget).data('id');
        var NewOrEdit ="New";
        var id_transaksi="<?php echo "$id_transaksi";?>";
        $.ajax({
            url     : "_Page/Retur/DetailRincianRetur.php",
            method  : "POST",
            data    : { id_rincian_retur: id_rincian_retur },
            success: function (data) {
                $('#DetailRincianRetur').html(data);
                //Ketika tombol hapus di click
                var id_transaksi="<?php echo "$id_transaksiRetur123";?>";
                $('#HapusRincianRetur').click(function(){
                $('#NotifikasiHapusRincianRetur').html('Loading..');
                var id_transaksi="<?php echo "$id_transaksiRetur123";?>";
                $.ajax({
                    url     : "_Page/Retur/ProsesHapusRincianRetur.php",
                    method  : "POST",
                    data    : { id_rincian_retur: id_rincian_retur, id_transaksi: id_transaksi },
                    success: function (data) {
                        $('#NotifikasiHapusRincianRetur').html(data);
                        var id_transaksi123="<?php echo "$id_transaksiRetur123";?>";
                        $.ajax({
                            type 	: 'POST',
                            url 	: '_Page/Retur/Retur.php',
                            data 	:  { id_transaksi: id_transaksi123 },
                            success : function(data){
                                $('#Halaman').html(data);
                            }
                        });
                        $('#ModalRincianRetur').modal('hide');
                    }
                })
            });
            }
    ...
    //Menampilkan Tabel rincian retur secara default
    var id_transaksi= "<?php echo "$id_transaksi";?>";
    $.ajax({
        type 	: 'POST',
            ...
            <div class="card-footer" id="NotifikasiSimpanFakturRetur">
                <?php
                    if(empty($id_transaksi)){
                        echo '<b class="text-danger">Keterangan :</b> Silahkan Pilih Transaksi Terlebih Dahulu.';
                    }else{
                        echo '<b class="text-primary">Keterangan :</b> Faktur Belum Disimpan.';
                    }
                ?>
